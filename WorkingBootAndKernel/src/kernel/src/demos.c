...
static void cow_parent_thread(void) {
    ...
    if (child_pid == 0) {
        ...
        uart_puts("[COW Parent] COW test complete! Press ESC to continue.\n");
        vga_write("[Parent] COW test complete! Press ESC.\n");
    }
}
...
static void signal_sender_thread(void) {
    ...
    pcb_t* receiver = find_process_by_pid(2); /* Assume receiver is PID 2 */
    if (!receiver) {
    ...
    uart_puts("[Sender] Sending SIGUSR1 to receiver...\n");
...
static void signal_receiver_thread(void) {
    ...
    uart_puts("[Receiver] Handler installed. Waiting for signal...\n");
...
void run_combined_cow_signals_demo(void) {
    ...
    if (!demo_stop_requested) {
        ...
        for (volatile int i = 0; i < 30000000 && !demo_stop_requested; i++) {
            __asm__ volatile("pause");
        }
    }
    ...
    if (!demo_stop_requested) {
        ...
        for (volatile int i = 0; i < 30000000 && !demo_stop_requested && !signal_received_flag; i++) {
            __asm__ volatile("pause");
        }
        
        /* Give a bit more time for cleanup messages */
        for (volatile int i = 0; i < 5000000 && !demo_stop_requested; i++) {
            __asm__ volatile("pause");
        }
    }
    ...
    /* Wait for ESC */
    demo_stop_requested = 0; /* Reset flag */
    while (!demo_stop_requested) {
        __asm__ volatile("pause");
    }
