 ...
 *
 * Note: the UDP layer dispatches by dst_port, so src_port in the
 * callback is the *remote* port.  The destination port that matched
 * is the one we registered, which we recover from port_map.
 */
