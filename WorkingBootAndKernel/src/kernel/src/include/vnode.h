...
/* Maximum filename length (short names for ramfs dir_entry, long for VFS API) */
...
/**
 * @brief Ramfs directory entry - links a name to a child vnode (in-memory)
 */
struct dir_entry {
    char name[64];
    struct vnode *vnode;
...
/**
 * @brief VFS directory entry returned by readdir (filesystem-agnostic)
 */
struct vfs_dirent {
...
struct vnode {
    ...
    /* Directory children — used by ramfs (in-memory dir entries) */
